...
export default function createSweepStatus(onError, EPS) {
  ...
  function checkDuplicate() {
    ...
    status.forEach((node) => {
      ...
      if (prev) {
        if (
          samePoint(prev.from, current.from) &&
          samePoint(prev.to, current.to)
        ) {
          // Likely you have received error before during segment removal.
          onError(
            "Duplicate key in the status! This may be caused by Floating Point rounding error"
          );
        }
      }
      prev = current;
  ...
  function deleteSegments(lower, interior, sweepLinePos) {
    ...
    if (status._size !== prevCount - interior.length - lower.length) {
      ...
      onError(
        "Segments were not removed from a tree properly. Precision error?"
      );
    }
  }
